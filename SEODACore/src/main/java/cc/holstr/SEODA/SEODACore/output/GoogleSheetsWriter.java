...
public class GoogleSheetsWriter {
	...
	public ArrayList<OutputSheet> getUpdatedSheets() {
		...
		InputStream response = httpHelper.get("https://sheets.googleapis.com/v4/spreadsheets/" + currentSpreadsheet + "?&fields=sheets.properties");
		...
		for (JsonObject sheet : sheetsArray.getValuesAs(JsonObject.class)) {
				...
				OutputSheet temp = new OutputSheet(prop.getJsonNumber("sheetId").longValue(), prop.getString("title"), prop.getInt("index"));
		...
		System.out.println("GOOGLE SHEETS : Sheets objects received.");
		return sheets; 
	}
	
	
	public OutputSheet makeSheet(String title, int rows, int cols) {
		//create a new sheet in currentSpreadsheet, and return OutputSheet object with its info
		JsonObject newSheet = GoogleJsonGenerator.getNewSheetObject(title, rows, cols);
		InputStream response = httpHelper.post("https://sheets.googleapis.com/v4/spreadsheets/" + currentSpreadsheet + ":batchUpdate", newSheet.toString());
		if(response!=null) {
			System.out.println("GOOGLE SHEETS SUCCESSFUL : new sheet called "+title+" added.");
		}
		return OutputSheet.create(Json.createReader(response).readObject()); 
		}
	
	public void deleteSheet(OutputSheet sheet) {
		//delete a sheet from currentSpreadsheet, and return OutputSheet object with its info
		JsonObject deleteSheet = GoogleJsonGenerator.getDeleteSheetObject(sheet.getId());
		InputStream response = httpHelper.post("https://sheets.googleapis.com/v4/spreadsheets/" + currentSpreadsheet + ":batchUpdate",deleteSheet.toString());
		if(response!=null) {
			System.out.println("GOOGLE SHEETS SUCCESSFUL : sheet called " +sheet.getTitle() + " deleted.");
		}
	}
	...
	public void clearSheet(OutputSheet sheet) {
		...
		if(response!=null) {
			System.out.println("GOOGLE SHEETS SUCCESSFUL : Sheet " +sheet.getTitle() +" cleared.");
		}
	}
	
	public void writeRange(String sheetTitle, String range, String valueInputOption, String[][] writeVals) {
		//set a specified range from currentSpreadsheet to provided string matrix.
		 ...
		 */
		int terms = 0;
		String url = "https://sheets.googleapis.com/v4/spreadsheets/" + currentSpreadsheet + "/values/" + sheetTitle + "!" + range + "?valueInputOption=" + valueInputOption;
		...
		JsonObjectBuilder write = Json.createObjectBuilder()
				.add("range", sheetTitle + "!" + range)
				.add("majorDimension", "ROWS")
				.add("values", values);
		InputStream response = httpHelper.put(url, write.build().toString());
		if(response!=null) {
			System.out.println("GOOGLE SHEETS SUCCESSFUL : wrote " + terms + " cells to " + sheetTitle + "!" + range + ".");
		}
	}
	
	public String[][] readRange(String sheetTitle, String range) {
		//retrieve a specified range from currentSpreadsheet, return string matrix with its data.
		//example usage: readRange("Sheet1","A1:D4");
		boolean success = true; 
		List<ArrayList<String>> data = new ArrayList<ArrayList<String>>(); 
		String[][] vals;
		int terms= 0,nulls = 0;
		String url = "https://sheets.googleapis.com/v4/spreadsheets/" + currentSpreadsheet + "/values/" + sheetTitle + "!" + range;
		...
		if(success) {
			System.out.println("GOOGLE SHEETS SUCCESSFUL : read " + terms + " cells from " + sheetTitle + "!" + range + ".");
		}
 		return vals; 
	}
...
//				{"test3","test4"}
//		};
//		writeRange("Sheet1","A1:B2","USER_ENTERED",temp);
		String[][] temp = readRange("Sheet1","A1:D4");
		for(String[] row : temp) {
			for(String col : row) {
